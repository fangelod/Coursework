...
static void make_r2r(float mat[6], const GBitmap& src, const GRect& dst) {
    mat[0] = dst.width() / src.width();
    mat[1] = 0;
    mat[2] = dst.left();
    mat[3] = 0;
    mat[4] = dst.height() / src.height();
    mat[5] = dst.top();
}
...
public:
    BitmapShape(const GBitmap& bm, const GIRect& center) : fBM(bm), fCenter(center) {
    ...
    void onDraw(GCanvas* canvas) override {
        if (fCenter.isEmpty()) {
            float mat[6];
            make_r2r(mat, fBM, fRect);
            GShader* a = GShader::FromBitmap(fBM, mat);
            GPoint pts[] = {
                { fRect.right(), fRect.top() },
                { fRect.left(), fRect.bottom() },
            };
            const GColor colors[] = {
            ...
            GShader* c = GShader::FromLerp(a, b, fT);
            canvas->shadeRect(fRect, c);
//            canvas->fillBitmapRect(fBM, fRect);
        } else {
            canvas->fillBitmapNine(fBM, fCenter, fRect);
        }
    }
    ...
    bool handleKey(int c) override {
        ...
        switch (c) {
            case 'a': fT = std::max(0.0f, fT - dt); return true;
            case 'b': fT = std::min(1.0f, fT + dt); return true;
        }
...
static GPoint ave(const GPoint& a, const GPoint& b) {
    return GPoint::Make(ave(a.fX, b.fX), ave(a.fY, b.fY));
}

static void chop(const GPoint src[3], GPoint dst[5]) {
    dst[0] = src[0];
    dst[1] = ave(src[0], src[1]);
    dst[3] = ave(src[1], src[2]);
    dst[4] = src[2];
    dst[2] = ave(dst[1], dst[3]);
}

static GPoint* subdivide(const GPoint src[], GPoint dst[], int level) {
    if (level > 0) {
        GPoint tmp[5];
        chop(src, tmp);
        dst = subdivide(&tmp[0], dst, level - 1);
        dst = subdivide(&tmp[2], dst, level - 1);
    } else {
        memcpy(dst, src, 2 * sizeof(GPoint));
        dst += 2;
    }
    return dst;
}

static void stroke_quad(GCanvas* canvas, const GPoint pts[], bool isClosed,
                        const GCanvas::Stroke& stroke, GShader* shader) {
    const int pow2 = 5;
    const int N = (1 << (pow2 + 1)) + 1;
    GPoint dst[N];

    GPoint* end = subdivide(pts, dst, pow2);
    *end++ = pts[2];
    GASSERT(dst + N == end);

    canvas->strokePolygon(dst, N, isClosed, stroke, shader);
}
...
public:
    ...
    void onDraw(GCanvas* canvas) override {
        ...
        if (fIsQuad) {
            GCanvas::Stroke stroke = { fWidth, 0, fAddCap };
            stroke_quad(canvas, fPts, false, stroke, shader);
        } else {
    ...
    static void show_box(GCanvas* canvas, const GPoint& p, float r, const GColor& color) {
        canvas->fillRect(GRect::MakeLTRB(p.fX - r, p.fY - r, p.fX + r, p.fY + r), color);
    }
    ...
    void drawHilite(GCanvas* canvas) override {
        ...
        for (auto p : fPts) {
            show_box(canvas, p, r, c);
        }
        if (fIsQuad && fShowSkeleton) {
            const GCanvas::Stroke stroke{ 2, 0, false };
            canvas->strokeLine(fPts[0], fPts[1], stroke, c);
            canvas->strokeLine(fPts[2], fPts[1], stroke, c);
            if (2 == fShowSkeleton) {
                GPoint ab = ave(fPts[0], fPts[1]),
                       bc = ave(fPts[1], fPts[2]),
                       abc = ave(ab, bc);
                canvas->strokeLine(ab, bc, stroke, c);
                show_box(canvas, ab, r, c);
                show_box(canvas, bc, r, c);
                show_box(canvas, abc, r, c);
            }
        }
    }
    ...
    bool handleKey(int c) override {
        switch (c) {
            ...
            case 'q': fIsQuad = !fIsQuad; return true;
            case 'k': fShowSkeleton = (fShowSkeleton + 1) % 3; return true;
        }
        return this->INHERITED::handleKey(c);
    }
...
static Shape* cons_up_shape(int index) {
    const struct {
        ...
        { "apps/spock.png", GIRect::MakeXYWH(0, 0, 0, 0) },
        { "apps/newpatch.png", GIRect::MakeXYWH(27, 27, 1, 1) },
    };
    GBitmap bm;
    if (index < GARRAY_COUNT(recs) && bm.readFromFile(recs[index].fName)) {
        return new BitmapShape(bm, recs[index].fCenter);
    }
    return NULL;
}
