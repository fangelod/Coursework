...
////////////////////////////////////////////////////////////////////////////////
     ...
     *  center rect (which encloses section 4).
     *
     *  -------------               ---------------------------------
     *  | 0 | 1 | 2 |               | 0 |           1           | 2 |
     *  -------------               ---------------------------------
     ...
     */
    void fillBitmapNine(const GBitmap& bm, const GIRect& center, const GRect& dst) {
    	/**
    	 *	- You can assume that center will have sensible values
    	 *		- 0 <= center.left() <= center.right() <= bm.width()
    	 *		- 0 <= center.top() <= cetner.bottom() <= bm.height()
    	 *	- Note: Any of the 9 sections could be empty (i.e. have either a zero width or height).
    	 */

    	// Fill 9 sections independently
    	 	// Fill 4 corners first
    	 	// Fill other sections

    	// Get dimensions of bm, center & dst
    	 	// bm
    	int bW = bm.width();
    	int bH = bm.height();
    	...
    	int dB = dst.bottom();
    		...
    		// apply CTM to dst
    	dL = CTM[0]*dL + CTM[1]*dT + CTM[2];
    	dT = CTM[3]*dL + CTM[4]*dT + CTM[5];
    	dR = CTM[0]*dR + CTM[1]*dB + CTM[2];
    	dB = CTM[3]*dR + CTM[4]*dB + CTM[5];

    	//printf("dst: (%i, %i, %i, %i)\n", dL, dT, dR, dB);
    	//printf("draw.fWidth = %i\n", draw.width());
    	//printf("rowBytes = %lu\n", draw.rowBytes());
    	//printf("\n");

    	// Fill Section 0 WORKING
    	{
    		// width: cL
    	 	// height: cT
			...
			// Find necessary scale factor for both width and height
			float sx = (float)sW/(float)w;
			float sy = (float)sH/(float)h;

			float tx = 0 - l;
			float ty = 0 - t;

			// Combine the matrices
			float both[6] = {sx, 0, sx*tx, 0, sy, sy*ty};

			// For loop that takes each point in dst and find corresponding point in src
			GPixel* d = draw.fPixels;
			d = (GPixel*)((char*)d + (int)t * draw.rowBytes());
			for (int y = t; y < b; ++y) {
				for (int x = l; x < r; ++x) {
					// Only map pixels that are in the bitmap
					if ((x >= 0 && x < draw.width()) && (y >= 0 && y < draw.height())) {
						...
						// Prepare color to fill
						unsigned a = GPixel_GetA(*bm.getAddr(xP, yP));
						unsigned r = GPixel_GetR(*bm.getAddr(xP, yP));
						unsigned g = GPixel_GetG(*bm.getAddr(xP, yP));
						unsigned b = GPixel_GetB(*bm.getAddr(xP, yP));
						...
						if (GPixel_GetA(*draw.getAddr(x, y)) > 0) {
		    	    		// blend
		    	    			// Get a, r, g, b from bitmap
							float lA = GPixel_GetA(*bm.getAddr(xP, yP)) / 255.99999;
							float lR = GPixel_GetR(*bm.getAddr(xP, yP)) / 255.99999;
							float lG = GPixel_GetG(*bm.getAddr(xP, yP)) / 255.99999;
							float lB = GPixel_GetB(*bm.getAddr(xP, yP)) / 255.99999;
