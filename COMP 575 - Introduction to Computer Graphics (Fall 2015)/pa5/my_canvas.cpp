...
////////////////////////////////////////////////////////////////////////////////
	...
	void shadeConvexPolygon(const GPoint po[], int count, GShader* shader) {
		...
		for (int y = startY; y < endY; ++y) {
			...
			shader->shadeRow(lrBound(eArr, currL, y), y, lrBound(eArr, currR, y)-lrBound(eArr, currL, y), store);
		...
		for (int i = 0; i < n; i++) {
			if (i == 0) { // p[n-1] -> p[0] <- p[1]
				if (isClosed == true) {
					...
					float ldx = ldx * lrad / llen;
					float ldy = ldy * lrad / llen;
					...
					float rdx = rdx * rrad / rlen;
					float rdy = rdy * rrad / rlen;
					GPoint rt = GPoint::Make((-1*rdy), rdx);

					if (s.fAddCap == true || h > s.fMiterLimit) {
						// Bevel cap
